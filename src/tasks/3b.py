...
from src.utils.projecoes_camera import transformarPontosInteresse, definirBaseCamera, mundoParaCamera
...
baseDaCamera, baseDoMundoNew, atNew = transformarPontosInteresse(eye, at, u, v, n)

# ======================================
# Adicionando o base do scm e scc
# ======================================
desenharBase(eixoDaCamera, baseDaCamera, baseDoMundoNew, labels=np.array(["x mundo", "y mundo", "z mundo"]), scatterLabel="Mundo", colorSet=1)
desenharBase(eixoDaCamera, np.array([0,0,0]), np.array([[1,0,0],[0,1,0],[0,0,1]]), labels=np.array(["x camera", "y camera", "z camera"]), scatterLabel="Camera")

eixoDaCamera.scatter(atNew[0], atNew[1], atNew[2], color='purple', s=100, label='at new')
eixoDaCamera.legend()

plotar3dimensoes(objetos=objetosDoSistemaDaCamera, eixo = eixoDaCamera)
